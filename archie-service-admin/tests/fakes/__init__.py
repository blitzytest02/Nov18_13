...
This package provides in-memory fake implementations of all repository interfaces
used by the Figma integration feature. These fakes enable fast, deterministic unit
testing without requiring database connections, Google Secret Manager access, or
external API calls.

Architecture:
    The fakes follow the Test Double pattern as documented in
    archie-service-backend/docs/TEST.md, implementing repository interfaces defined
    in src/repositories/interfaces/. Each fake maintains internal state using Python
    dictionaries and lists, simulating the behavior of external systems without
    actual dependencies.
...
Available Fakes:
    FakeFigmaRepository: Simulates database operations for Figma installations,
        access control records, and frame attachments. Supports soft deletes,
        unique constraints, and idempotent operations.

    FakeSecretRepository: Simulates Google Cloud Secret Manager operations for
        PAT storage and retrieval. Supports error injection for testing transaction
        rollback scenarios.
    ...
    FakeConfigRepository: Simulates application configuration access. Enables
        testing of feature flag behavior and configuration-dependent logic without
...
State Management:
    ...
    Each fake provides a reset() method to clear state between tests. For maximum
    isolation, create new fake instances for each test or use pytest fixtures
    with function scope.

Thread Safety:
    These fakes are NOT thread-safe and should only be used in single-threaded
    test environments. Each test should create its own fake instances or properly
    isolate state to avoid interference between parallel tests.
