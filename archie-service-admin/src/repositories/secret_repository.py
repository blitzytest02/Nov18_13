...
class SecretRepository(ISecretRepository):
    ...
    def create_secret(
        ...
        while attempt < retry_count:
            try:
                ...
                raise SecretManagerError(
                    f"Permission denied: Service account lacks roles/secretmanager.admin for project '{self._project_id}'",
                    original_exception=e,
                    operation="create_secret"
                )
                ...
                if attempt < retry_count:
                    ...
                    delay = 2 ** (attempt - 1)
                    logger.warning(
                        f"Transient error creating secret {secret_name} (attempt {attempt}/{retry_count}): {e}. "
                        f"Retrying in {delay}s..."
                    )
                    time.sleep(delay)
                else:
                    logger.error(f"Failed to create secret {secret_name} after {retry_count} attempts")
                    raise SecretManagerError(
                        f"Failed to create secret '{secret_name}' after {retry_count} attempts: {e}",
                        original_exception=e,
                        operation="create_secret"
                    )
    ...
    def get_secret(self, secret_name: str) -> Optional[str]:
        ...
        Secret Naming Pattern:
            Must use pattern 'figma-secret-<installation_id>'. Example: get_secret('figma-secret-42')
            retrieves the PAT for figma_installation with id=42.
        ...
        try:
            ...
            raise SecretManagerError(
                f"Permission denied: Service account lacks roles/secretmanager.secretAccessor for '{secret_name}'",
                original_exception=e,
    ...
    def update_secret(
        ...
        while attempt < retry_count:
            try:
                ...
                logger.info(f"Secret updated successfully: {secret_name} (new version: {version.name})")
                return True
                ...
                raise SecretManagerError(
                    f"Permission denied: Service account lacks permissions to update '{secret_name}'",
                    original_exception=e,
                    operation="update_secret"
                )
                ...
                if attempt < retry_count:
                    ...
                    delay = 2 ** (attempt - 1)
                    logger.warning(
                        f"Transient error updating secret {secret_name} (attempt {attempt}/{retry_count}): {e}. "
                        f"Retrying in {delay}s..."
                    )
                    time.sleep(delay)
                else:
                    logger.error(f"Failed to update secret {secret_name} after {retry_count} attempts")
                    raise SecretManagerError(
                        f"Failed to update secret '{secret_name}' after {retry_count} attempts: {e}",
                        original_exception=e,
                        operation="update_secret"
                    )
    ...
    def delete_secret(
        ...
        while attempt < retry_count:
            try:
                ...
                logger.info(f"Secret {secret_name} not found (already deleted or never existed)")
                return True
                ...
                raise SecretManagerError(
                    f"Permission denied: Service account lacks permissions to delete '{secret_name}'",
                    original_exception=e,
                ...
                if attempt < retry_count:
                    ...
                    delay = 2 ** (attempt - 1)
                    logger.warning(
                        f"Transient error deleting secret {secret_name} (attempt {attempt}/{retry_count}): {e}. "
                        f"Retrying in {delay}s..."
                    )
                    time.sleep(delay)
                else:
                    logger.error(f"Failed to delete secret {secret_name} after {retry_count} attempts")
                    raise SecretManagerError(
                        f"Failed to delete secret '{secret_name}' after {retry_count} attempts: {e}",
                        original_exception=e,
                        operation="delete_secret"
                    )
